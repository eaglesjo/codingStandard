...
function Invoke-Installer {
    param([string]$Target,[ValidateSet("en", "ko")][string]$Language,[ValidateSet("common", "llm", "vision", "all")][string]$Domain,[ValidateSet("Ask", "Merge", "Overwrite", "Skip")][string]$ConflictAction = "Overwrite",[switch]$DryRun)
...
try {
    ...
    $spaceTarget = Join-Path $root "한국어 공백 경로"; New-Item -ItemType Directory -Force -Path $spaceTarget | Out-Null; Invoke-Installer -Target $spaceTarget -Language ko -Domain vision
    if (-not (Test-Path (Join-Path $spaceTarget "domains/vision/AGENT.md"))) { throw "Installation failed in Unicode/space path" }

...
def run_environment_tests() -> None: run_checker(ROOT / "scripts/development/test_environment.py", "Environment detection tests")
...
def check_notebook() -> None:
    ...
    if notebook.get("nbformat") != 4 or not notebook.get("cells"): fail("Invalid or empty Colab notebook")
    ...
    for required in ("import subprocess", "import sys", "import platform", "subprocess.run", "sys.executable", "platform.platform", "DEFAULT_REPO_URL", "_normalize_repository", "GITHUB_TOKEN", "GIT_ASKPASS"):
        if required not in code: fail(f"Colab notebook missing required symbol: {required}")
    ...
    for root in (ROOT / "core", ROOT / "domains"):
        for path in root.rglob("*.md"):
            for pattern in FORBIDDEN_HARDWARE_PATTERNS:
                if pattern.search(path.read_text(encoding="utf-8")): fail(f"Machine-specific hardware assumption in {path}: {pattern.pattern}")
...
def check_windows_workflow() -> None:
    ...
    for required in ("powershell", "pwsh", "-DryRun", "-ConflictAction Merge"):
        if required.lower() not in (workflow + "\n" + test_script).lower(): fail(f"Windows validation missing: {required}")


def check_version_consistency() -> None:
    version = (ROOT / "VERSION").read_text(encoding="utf-8").strip()
    if not re.fullmatch(r"\d+\.\d+\.\d+", version): fail(f"VERSION must use semantic versioning: {version!r}")
    text = (ROOT / "core/common/environment.py").read_text(encoding="utf-8")
    match = re.search(r'STANDARD_VERSION\s*=\s*["\']([^"\']+)["\']', text)
...
def run_i18n_check() -> None: run_checker(ROOT / "scripts/validation/check_i18n.py", "English/Korean localization check")


def main() -> None:
    check_required_files(); run_structure_check(); check_python(); run_environment_tests(); check_notebook(); check_hardware_neutrality(); check_no_legacy_installer(); check_windows_workflow(); check_version_consistency(); run_i18n_check(); print("codingStandard validation passed")

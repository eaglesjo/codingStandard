...
REQUIRED_FILES = [
    "VERSION", "LICENSE", "AGENTS.md", "CLAUDE.md", "GEMINI.md",
    "COMMON/AGENT.md", "COMMON/SKILL.md", "COMMON/ENVIRONMENT.md", "COMMON/environment.py", "COMMON/experiment.py",
    "LLM/AGENT.md", "LLM/SKILL.md", "LLM/ENVIRONMENT.md", "LLM/environment.py", "LLM/memory_smoke_test.py", "LLM/experiment.py",
    "VISION/AGENT.md", "VISION/SKILL.md", "VISION/ENVIRONMENT.md", "VISION/memory_smoke_test.py",
    "scripts/install-domains.ps1", "scripts/install-domains.sh", "scripts/check_i18n.py", "scripts/test_installers.py", "scripts/test_installers_windows.ps1",
    ".github/workflows/windows-install-test.yml", "tests/colab/README.md", "tests/colab/codingstandard_colab_test.ipynb",
]
...
def check_notebook() -> None:
    ...
    if notebook.get("nbformat") != 4 or not notebook.get("cells"): fail("Invalid Colab notebook structure")
...
def run_i18n_check() -> None:
    ...
    if not callable(main) or main() != 0: fail("English/Korean localization check failed")
...
def check_windows_workflow() -> None:
    ...
    if "runs-on: windows-latest" not in workflow: fail("Windows workflow must use windows-latest")
    for required in ("powershell", "pwsh", "DryRun", "Merge"): 
        if required not in workflow: fail(f"Windows workflow missing: {required}")


def main() -> None:
    check_required_files(); check_python(); check_notebook(); check_hardware_neutrality(); check_no_legacy_installer(); check_windows_workflow(); run_i18n_check()

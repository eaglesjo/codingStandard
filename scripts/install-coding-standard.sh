...
case "$DRY_RUN" in true|false) ;; *) printf 'Invalid dry-run value: %s. Use true or false.\n' "$DRY_RUN" >&2; exit 1 ;; esac

get_source() {
  local relative="$1"; local localized="$SOURCE_ROOT/i18n/ko/$relative"; local english="$SOURCE_ROOT/$relative"
  if [[ "$LANGUAGE" == "ko" && -f "$localized" ]]; then printf '%s\n' "$localized"; return; fi
  [[ -f "$english" ]] || { printf 'Template not found: %s\n' "$relative" >&2; exit 1; }
  printf '%s\n' "$english"
}

conflict_choice() {
  local destination="$1" choice
...
merge_aider_config() {
  ...
  if grep -qE '^read:[[:space:]]*\[' <<< "$existing"; then
    ...
    awk '/^read:[[:space:]]*\[/ { sub(/\][[:space:]]*$/, ", CONVENTIONS.md]"); print; next } { print }' <<< "$existing"; return
  fi
  if grep -qE '^read:[[:space:]]*$' <<< "$existing"; then
    grep -q 'CONVENTIONS.md' <<< "$existing" && { printf '%s\n' "$existing"; return; }
    awk 'BEGIN{added=0} /^read:[[:space:]]*$/ && !added {print; print "  - CONVENTIONS.md"; added=1; next} {print}' <<< "$existing"; return
  fi
...
install_file() {
  local source_relative="$1" destination_relative="$2" source destination source_text existing_text action
...
INSTALL_MAP=(
  ...
  '.github/copilot-instructions.md|.github/copilot-instructions.md' '.github/instructions/llm.instructions.md|.github/instructions/llm.instructions.md'
  '.cursor/rules/coding-standard.mdc|.cursor/rules/coding-standard.mdc' '.windsurf/rules/coding-standard.md|.windsurf/rules/coding-standard.md'
  '.clinerules/01-coding-standard.md|.clinerules/01-coding-standard.md' '.continue/rules/01-coding-standard.md|.continue/rules/01-coding-standard.md'
  '.junie/AGENTS.md|.junie/AGENTS.md' '.amazonq/rules/coding-standard.md|.amazonq/rules/coding-standard.md'
  'CONVENTIONS.md|CONVENTIONS.md' '.aider.conf.yml|.aider.conf.yml'
  'LLM/AGENT.md|LLM/AGENT.md' 'LLM/SKILL.md|LLM/SKILL.md' 'LLM/ENVIRONMENT.md|LLM/ENVIRONMENT.md' 'LLM/environment.py|LLM/environment.py' 'LLM/README.md|LLM/README.md'
  'LLM/config/training.yaml|LLM/config/training.yaml' 'LLM/config/ablation.yaml|LLM/config/ablation.yaml'
  'LLM/experiment.py|LLM/experiment.py'
)

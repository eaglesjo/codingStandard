...
`LLM/ENVIRONMENT.md`는 실행환경 확인 및 최적화 규칙의 canonical source이다.
...
6. 작업 대상 디렉터리에 더 구체적인 지침 파일이 있으면 해당 지침
7. 프로젝트의 기존 `README.md`, `pyproject.toml`, lock file 및 테스트 규칙
...
- 실제 측정 결과와 workload를 기준으로 최종 runtime configuration을 확정한다.
- 특정 PC, GPU, RAM 용량을 현재 환경의 전제조건으로 하드코딩하지 않는다. 실제 환경은 프로파일러 결과를 기준으로 처리한다.
...
- 저사양/제한 VRAM·RAM 환경에서는 보수적인 memory budget을 우선한다.
...
전체 구현 규칙과 예제는 `LLM/AGENT.md`, `LLM/SKILL.md`, `LLM/ENVIRONMENT.md`를 기준으로 한다.

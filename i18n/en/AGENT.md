...
Do not document a specific machine as the required runtime environment.

Runtime decisions must be based on measured resources and workload requirements. Historical machine-specific information, when needed, belongs outside executable decision logic.
...
Do not use user-specific Windows paths or Unix-only shell commands as the default application path.
...
Before long training, run a minimal workload:
...
Record peak VRAM, peak RAM, metrics, runtime, and resolved profile. If it fails, lower the configuration before starting the main run.
...
Do not retry the exact same failing configuration indefinitely.
...
With Hugging Face Trainer, configure `EarlyStoppingCallback`, `metric_for_best_model`, `greater_is_better`, and `load_best_model_at_end` consistently.

## 18. Checkpoint / Resume

Training must be resumable when practical.
...
Keep the following controlled across variants whenever possible:
...
Repeated execution must not cause uncontrolled accumulation.
...
```text
[ ] Actual OS / Python / runtime checked
[ ] CPU / RAM checked
[ ] GPU / VRAM / accelerator checked
[ ] environment.py executed
[ ] environment profile generated
[ ] runtime configuration resolved
[ ] environment locked
[ ] unused environment branches removed
[ ] Memory Smoke Test passed
[ ] OOM recovery strategy verified
[ ] validation metric
[ ] Early Stopping
[ ] best checkpoint / Resume
[ ] ablation matrix
[ ] controlled comparison
[ ] metric / VRAM / RAM / runtime recorded

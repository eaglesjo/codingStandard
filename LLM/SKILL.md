...
For supported accelerators, evaluate mixed precision. Use BF16 only after verifying actual hardware/framework support.
...
Record peak VRAM, peak RAM, validation metric, runtime, and resolved environment profile. If it fails, lower the configuration before starting the main run.
...
Adapt values to the actual project and workload.
...
Require validation, an explicit metric/direction, patience, best checkpoint saving/restoration, and event logging.
...
Keep train/validation split, test set, metric, Early Stopping policy, maximum budget, checkpoint rule, seed set, and resolved environment profile controlled across variants whenever possible.

## 13. Experiment Tracking

Record experiment ID, variant, changed parameters, seed, model/dataset revisions, best metric, best epoch/step, early-stopped status, peak VRAM/RAM, runtime, checkpoint path, and resolved environment profile.
